...
<main>
  <!-- HG_TYPO_TABLE -->
    ...
    const config = {
      fontFamilyMap: {
        'primary-light': '"suisse-light", sans-serif'
      },
      typo: {
        'title-l': {
    ...
    const transformer = new ThemeTransformer(projectRoot);
    const ok = transformer.transform(srcPath, outPath, 'dutti', true, config, null, null);
  ...
  try {
    ...
    const transformer = new ThemeTransformer(projectRoot);
    const ok = transformer.transform(srcPath, outPath, 'dutti', true, null, null, null);
