...
function generateHTML(configData) {
  ...
  const tableRows = classNames.map(className => {
    ...
    return `
      <tr>
        ...
        <td class="mobile-value">${cls.mobile?.fontSize ? pxToRem(cls.mobile.fontSize, baseFontSize) : '-'}</td>
        <td class="mobile-value">${cls.mobile?.lineHeight || '-'}</td>
        <td class="desktop-value">${cls.desktop?.fontSize ? pxToRem(cls.desktop.fontSize, baseFontSize) : '-'}</td>
